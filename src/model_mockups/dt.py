...
###############################################################################
## Create learning model (Decision Tree), fit model, analyze results
###############################################################################
...
for md in range (2, 25):
  lr = DecisionTreeClassifier (max_features = 10, max_depth = md)
  lr = lr.fit (X_train, y_train)

  y_test_pred = lr.predict (X_test)
  dtAccuracy = accuracy_score (y_test, y_test_pred)

  print (str ('MD = %2d' % md) + ' ' +
         str ('Accuracy = %f %%' % (100* dtAccuracy)))
